...
/**
 * Returns deduplicated download CDN base URLs.
 * config.download = ["https://cf.streamflixserver.site/", ...] — works without auth.
 * config.premium = Wasabi S3 (403 Forbidden).
 * config.movies/tv = bb.streamflixserver.site (unreachable/hung).
 */
...
/** Detect embedded subtitles from the filename (e.g. "ESub", ".srt", ".ass"). */
...
export async function fetchStreamflixStreams(
  ...
  try {
    ...
    try {
      ...
      // Firebase episodes are 0-indexed
    ...
    // Fallback URL pattern when Firebase data is unavailable
    const base = bases[0];
