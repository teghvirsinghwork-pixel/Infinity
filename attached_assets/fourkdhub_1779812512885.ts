...
function sizeFromHtml(html: string): string {
  // Handles both live badges and commented-out ones: <!-- ... 65.95 GB ... -->
...
async function findPageUrl(
  ...
  if (best) {
    logger.info(`[${PROVIDER}] found page: ${best.href} (dist=${best.dist})`);
    return best.href;
  }
...
function parseFileEntries($: cheerio.CheerioAPI): FileEntry[] {
  ...
  // New structure: id="content-file{N}" divs
...
/** For series: filter entries that match the requested S/E. */
...
/**
 * hubdrive.space/file/{id} pages embed a direct link we can extract.
 * If the page has an `<a>` pointing to hubcloud or a direct media link, use it.
 * Otherwise fall back to the hubdrive URL itself.
 */
async function resolveHubDrive(url: string): Promise<string | null> {
  try {
    const $ = await fetchHtml(url);
    if (!$) return url; // best-effort: return original

    // Look for a HubCloud redirect on the page
    const hcLink = $("a[href*='hubcloud']").first().attr("href");
    if (hcLink) return hcLink;

    // Look for any direct download button
    const dlLink = $("a[href]")
...
export async function getFourkdHubStreams(
  ...
  // For series, filter to the requested S/E
    ...
    entries.map(async (entry) => {
      ...
      if (entry.hubDriveUrl && results.length === 0) {
        try {
          ...
          if (resolved) {
            // If resolved to a hubcloud URL, run full extraction
