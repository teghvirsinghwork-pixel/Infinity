...
import { getStreams as dooflixStreams } from "../providers/dooflix.js";
...
async function getDooflixStreams(
  ...
  try {
    ...
    if (!tmdbId) {
      logger.warn({ imdbId }, "DooFlix: could not resolve TMDB ID");
      return [];
    }
    const mediaType = type === "series" ? "tv" : "movie";
    return await dooflixStreams(tmdbId, mediaType, season, episode);
  } catch (err) {
    logger.error({ err, imdbId }, "DooFlix: provider error");
    return [];
  }
}
...
async function getDooflixStreamsFromTmdb(
  tmdbId: string,
  type: string,
  season?: number,
  episode?: number,
  ...
  try {
    ...
    logger.error({ err, tmdbId }, "DooFlix (TMDB): provider error");
    return [];
  }
}
...
router.get("/stream/:type/:id.json", async (req, res) => {
  ...
  try {
    ...
    if (id.startsWith("tt")) {
      ...
      logger.info({ imdbId, title: meta.title, year: meta.year }, "Stremio: IMDB — querying 9 providers");
      ...
      const [asResult, raResult, adResult, nmResult, dfResult, mbResult, hmResult, hdResult, zmResult] = await Promise.allSettled([
        ...
        ep.has("dooflix") ? getDooflixStreams(imdbId, type, season, episode) : Promise.resolve([]),
      ...
      const dfStreams = dfResult.status === "fulfilled" ? dfResult.value : [];
      const mbStreams = mbResult.status === "fulfilled" ? mbResult.value : [];
      ...
      const raw = dedup(([...asStreams, ...raStreams, ...adStreams, ...nmStreams, ...dfStreams, ...mbStreams, ...hmStreams, ...hdStreams, ...zmStreams]) as Record<string, unknown>[]);
      const combined = premiumFormat(raw, meta.title, contentType, season, episode);
      logger.info(
        { imdbId, title: meta.title, as: asStreams.length, ra: raStreams.length, ad: adStreams.length, nm: nmStreams.length, df: dfStreams.length, mb: mbStreams.length, hm: hmStreams.length, hd: hdStreams.length, zm: zmStreams.length, combined: combined.length },
        "Stremio: 9 providers aggregated",
      );
      logResolve({ imdbId, step: "done", status: combined.length ? "ok" : "fail", detail: `as=${asStreams.length} ra=${raStreams.length} ad=${adStreams.length} nm=${nmStreams.length} df=${dfStreams.length} mb=${mbStreams.length} hm=${hmStreams.length} hd=${hdStreams.length} zm=${zmStreams.length} total=${combined.length}` });
    ...
    if (id.startsWith("tmdb:")) {
      ...
      logger.info({ tmdbId: numericTmdbId, imdbId: meta.imdbId, title: meta.title }, "Stremio: TMDB — querying 9 providers");
      ...
      const [asResult, raResult, adResult, nmResult, dfResult, mbResult, hmResult, hdResult, zmResult] = await Promise.allSettled([
        ...
        ep2.has("dooflix") ? getDooflixStreamsFromTmdb(numericTmdbId, type, season, episode) : Promise.resolve([]),
      ...
      const raw2 = dedup(([...asStreams, ...raStreams, ...adStreams, ...nmStreams, ...dfStreams, ...mbStreams, ...hmStreams, ...hdStreams, ...zmStreams]) as Record<string, unknown>[]);
      const combined = premiumFormat(raw2, meta.title, contentType, season, episode);
      logger.info(
        { tmdbId: numericTmdbId, title: meta.title, as: asStreams.length, ra: raStreams.length, ad: adStreams.length, nm: nmStreams.length, df: dfStreams.length, mb: mbStreams.length, hm: hmStreams.length, hd: hdStreams.length, zm: zmStreams.length, combined: combined.length },
        "Stremio: TMDB 9 providers aggregated",
      );
      logResolve({ imdbId: id, step: "done", status: combined.length ? "ok" : "fail", detail: `as=${asStreams.length} ra=${raStreams.length} ad=${adStreams.length} nm=${nmStreams.length} df=${dfStreams.length} mb=${mbStreams.length} hm=${hmStreams.length} hd=${hdStreams.length} zm=${zmStreams.length} total=${combined.length}` });
